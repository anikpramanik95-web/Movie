...
import { Film, Download, Trash2, Plus, LogIn, LogOut, X, Search, MessageSquarePlus, MessageSquare, Loader2 } from 'lucide-react';
  ...
  const handleUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Upload failed: ${errorData?.error || 'Please check your passcode or image size.'}`);
          ...
          {isAdmin ? (
            <div className="bg-[#111111] border border-dashed border-[#333] p-5 rounded-lg">
              ...
              <button 
                ...
                className="w-full bg-[#1a1a1a] text-white border border-[#333] font-bold py-2.5 rounded text-xs uppercase hover:bg-[#222] transition-colors flex items-center justify-center gap-2 relative"
              >
      ...
      <main className="flex-1 p-6 md:p-10 flex flex-col gap-8 overflow-y-auto">
        ...
        <div className="md:hidden flex justify-between items-center mb-4">
          ...
          {isAdmin ? (
            ...
            <button onClick={() => setShowLogin(true)} className="text-xs text-[#d4af37]">Admin Login</button>
          )}
        </div>
              ...
              <div key={movie.id} className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl overflow-hidden flex flex-col">
                 ...
                 <div className="p-5 flex-1 flex flex-col gap-3">
                   ...
                   <div className="flex gap-2 mt-auto pt-2">
                     <a href={movie.downloadUrl} target="_blank" rel="noopener noreferrer" className="flex-1 bg-[#1a1a1a] hover:bg-[#222] text-white text-center py-2.5 px-3 text-xs rounded border border-[#333] transition-colors flex items-center justify-center gap-2">
        ...
        <footer className="mt-auto pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 border-t border-[#1a1a1a] pb-4">
          ...
          <div className="text-[10px] text-[#666] uppercase tracking-widest flex items-center gap-2">
            <span>Contact:</span>
            <a href="mailto:pramanikanik447@gmail.com" className="text-[#d4af37] hover:text-white transition-colors border-b border-[#bf0c0c]">
              pramanikanik447@gmail.com
            </a>
          </div>
        </footer>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl p-8 w-full max-w-sm relative shadow-2xl">
            ...
            <form onSubmit={handleLogin}>
              <div className="mb-6">
                <label className="block text-xs uppercase tracking-wider text-[#888] mb-2">Passcode</label>
                <input 
                  type="password" 
                  value={adminPasscode}
              ...
              <button 
                type="submit"
                className="w-full bg-[#d4af37] text-black font-bold py-3 rounded text-sm uppercase transition-colors hover:bg-[#b5952f]"
              >
                Access Dashboard
              </button>
            </form>
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl p-8 w-full max-w-md relative shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleUpload} className="space-y-5">
              ...
              <div>
                <label className="block text-xs uppercase tracking-wider text-[#888] mb-2">Download Link *</label>
                <input 
                  type="text" 
